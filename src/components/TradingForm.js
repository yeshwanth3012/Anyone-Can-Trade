...
const TradingForm = ({ form }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch("https://api.tradingmastersindia.com//api/save-user-qa", {
        ...
        body: JSON.stringify({
          paymentId: paymentId,
          answers: JSON.stringify(masteryForm),
        }),
      });
  ...
  return (
    <>
      <div
        className={`rounded-lg py-5 px-4 sm:px-8 md:px-[9rem] flex flex-col gap-6 mx-auto shadow-sm ${
          form ? "w-[100%] bg-[#F0F0F0]" : "mt-[6.5rem] w-[95%] bg-[#E3C45D0D]"
        }`}
      >
          ...
          <div className="flex flex-col gap-6 md:pl-8">
            ...
            <div className="flex flex-col items-start">
              <p className="mb-1 text-start">
                With Our ACT Mastery Mentorship, we will make you a successful
                trader in 3 months, Will you pay 80K for the 1:1 mentorship.
              </p>
